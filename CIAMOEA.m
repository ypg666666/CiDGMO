    ...
    methods
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(Population)
                %% 因果图更新
                if mod(ceil(Problem.FE/Problem.N),30) == 0
                    [UG,DiG] = ULG_con(Population);
                    [LG,LG_adjacencyMatrix] = LLG_con(Population);
                    [DoubleLayerEdges,DiG] = BG_con(DiG,LG,Population,Problem);
                    UG = adjacency(DiG);
                ...
                %% 基于因果的遗传算法
                geneticOffspring = [];
                while length(geneticOffspring) < 20
                    Parents = select_P(VV);
                    ...
                    for LA = top50Nodes'
                        ...
                        if ~isempty(relatedLowerNodes)
                            ...
                            if ~isempty(top3Nodes)
                                ...
                                Parents = select_P(VV);
                                Off = OperatorGACCB(Problem,Population(Parents),DoubleLayerEdges,UG,Parents);
                                attentionOffspring = [attentionOffspring newSolution Off];
                             
                            end
                        else
                            %MatingPool = TournamentSelection(2,Problem.N,FrontNo,-CrowdDis);
                            attentionOffspring  = OperatorGACCB(Problem,Population(Parents),DoubleLayerEdges,UG,Parents);
...
function [result,DiG] = BG_con(DiG,LG,Population,Problem)
    data = Population.decs;
    Edges = table2array(DiG.Edges);
    Edges = Edges(:,1:2);
    numEdges = size(Edges, 1);
    % 提取第一列和第二列的数据
    first_column = Edges(:, 1);
    second_column = Edges(:, 2);
    % 找出第二列中不在第一列出现的唯一值
    effnodes = unique(setdiff(second_column, first_column));
    ...
    for n = 1:numEdges
        ...
        numSelectedSamples = floor(0.2 * numSamples);
    ...
    for n = 1:numEdges
        ...
        % 计算原始互信息
        originalMI = gcmi_cc(effectNodeData,causeNodeData);
        ...
        for sampleIdx = 1:numSamples
            % 复制原始数据
            newEffectNodeData = effectNodeData;
            
            % 给当前样本加上一个小的随机扰动
            newEffectValue = newEffectNodeData(sampleIdx) + 0.1 * randn();
            % 确保新值在原因节点的上下限范围内
            newEffectValue = max(Problem.lower(effectNode), min(Problem.upper(effectNode), newEffectValue));
            newEffectNodeData(sampleIdx) = newEffectValue;
            
            % 计算变化后的互信息
            newMI = gcmi_cc(newEffectNodeData,causeNodeData);
        ...
        % 建立双层图连接
        for idx = 1:length(selectedSampleIndices)
            DoubleLayerEdges = [DoubleLayerEdges; [effectNode selectedSampleIndices(idx)]];
        end
    ...
    result = [];
    sorted_matrix = sortrows(DoubleLayerEdges, 1);
    unique_values = unique(sorted_matrix(:, 1));
    for i = 1:length(unique_values)
        current_value = unique_values(i);
        % 提取当前唯一值对应的行
        rows_with_current_value = sorted_matrix(sorted_matrix(:, 1) == current_value, 2);
        % 去除第二列中的重复值
        unique_second_column = unique(rows_with_current_value);
        % 构造新的行，第一列为当前唯一值，第二列为合并后的第二列值
        new_rows = [repmat(current_value, length(unique_second_column), 1), unique_second_column];
        % 将新行添加到结果矩阵中
        result = [result; new_rows];
    end
end
